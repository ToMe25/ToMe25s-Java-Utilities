...
public class Config {
	 ...
	 * @param comment
	 */
	public void addConfig(String config, String name, String type, Object defaultValue, String comment) {
		try {
			addConfig(new File(cfgDir, config), name, type, defaultValue, comment);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @param comment
	 */
	public void addConfig(File config, String name, String type, Object defaultValue, String comment) {
		try {
			if (cfg.containsKey(name)) {
				System.err.println("There is already a Config Option with this Name!");
			} else {
				cfg.put(name, new ConfigValue(config, name, type, defaultValue, comment));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static void createConfig(File config) {
		try {
			...
			fiout.write(("# The " + config.getName().split("\\.")[config.getName().split("\\.").length - 2]
					+ " Configuration for the Asteroids Minigame.").getBytes());
